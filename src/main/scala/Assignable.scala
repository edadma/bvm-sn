...
class MutableSeqAssignable( seq: MutableSeq[Any], index: Int ) extends Assignable {
	def value = seq( index )
...
class MutableMapAssignable( map: MutableMap[Any, Any], key: Any ) extends Assignable
{
	def value = map( key )
