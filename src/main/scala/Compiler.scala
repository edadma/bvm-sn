...
class Compiler( constants: Map[String, Any], sysvars: Map[String, VM => Any],
						macros: Map[String, List[AST] => AST], dir: Mode = Forward, comments: Boolean = false ) {
	sealed trait Decl
	...
	case class FunctionDecl( first: Position,
													 ...
													 var entry: Int,
													 parts: LinkedHashMap[Position, FunctionPart]) extends Decl
	...
	private var code: ArrayBuffer[VMInst] = _//todo: this could be moved to Compilation object which could then be passed to emitExtension() along with AST
...
//					filter foreach _decls
	...
	private def unify( struc: StructureAST, pos: Position, namespaces: List[Namespace], vars: HashSet[String] = new
			HashSet ): Unit = {
...
//			println( ast )
	...
	//						case ContextVarResult => sys.error( "bug: should have been declared" )
						case DeclVarResult( VarDecl(_, true, false), _, fidx ) => problem( pos, s"illegal forward reference to symbol '$oname'" )
...
//							println( name )
				...
				case AssignmentExpressionAST( lhs, op, fmap, rhs ) =>
					...
					code += AssignmentInst( len, lhs map {case (p, _) => p} toVector, if (op == Symbol( "" )) null else op, fmap, rhs map {case (p, _) => p} toVector )
...
//						code += FailIfFalseInst
				...
				case ForExpressionAST( label, gen, body, els ) =>
					def forloop( gs: List[GeneratorExpressionAST] ): ExpressionAST =
						gs match {
							case Nil => body
							case hd :: tl => EveryExpressionAST( if (gs == gen) label else None, hd, Some(forloop(tl)), if (gs == gen) els else None )
						}
