...
//		}
	...
	protected def equal( ch: Char ) = (flags&Pattern.CASE_INSENSITIVE) == 0 && current == ch || (flags&Pattern.CASE_INSENSITIVE) > 0 && Character.toLowerCase(current) == Character.toLowerCase(ch)
				...
				if (l.isInstanceOf[String] || r.isInstanceOf[String])
					...
					l match {
						...
						case value : collection.Map[_, _] if r.isInstanceOf[Iterable[_]] =>
							push( value ++ r.asInstanceOf[Iterable[Vector[Any]]].map(v => (v(0), v(1))) )
	...
	protected def callIndirect( callable: Any, fpos: Position, apos: Position, ps: List[Position], argc: Int ) {
		callable match {
			...
			case a: Array[Any] =>
				...
				derefp match {
					case idx: Int if idx < 0 || idx >= a.length => problem( ps.head, s"array (of length ${a.length}) index out of range: $idx" )
			...
			case ms: MutableSeq[_] =>
				...
				derefp match {
					case idx: Int if idx < 0 || idx >= ms.length => problem( ps.head, s"sequence (of length ${ms.length}) index out of range: $idx" )
					case idx: Int => push( new MutableSeqAssignable(ms.asInstanceOf[MutableSeq[Any]], idx) )
			...
			case m: collection.Map[_, _] =>
				...
				push( m.asInstanceOf[collection.Map[Any, Any]] getOrElse(arg, undefined/*problem(apos, s"key not found: ${display(arg)}")*/) )
...
//				println( list, list map (_.getClass) )//dbg
			...
			case NativeMethod( o, m ) =>
				...
				m.find( cm => assignable(args, cm.getParameterTypes) ) match {
					case None => problem( apos, s"no methods with matching signatures for: ${m.head.getName}: ${args mkString ", "}" )
					case Some( cm ) =>
						push( cm.invoke(o, rewrap(args, cm.getParameterTypes): _*) )
				}
			case s: collection.Set[_] => push( s.asInstanceOf[collection.Set[Any]](derefp) )
...
//						case (x: Closure, "scala.Function2") => x.function2
	...
	private def subsequence( start: Int ) = if (start > ptr) new SubSequence(seq, ptr, start ) else new SubSequence(seq, start, ptr )
			...
			if (ip < code.length) {
				...
				code( loc ) match {
						...
						derefp match {
							case r: Record =>
								r.symbolMap get field match {
									case None => problem( apos, s"'$field' not a field of record '${r.name}'" )
									case Some( n ) => push( r.element(n) )
								}
							case m: MutableMap[_, _] =>
							...
							case m: collection.Map[_, _] =>
								push( m.asInstanceOf[collection.Map[Any, Any]] getOrElse(field.name, undefined /*problem(apos, s"key not found: ${field.name}")*/) )
					...
					case PushCaptureGroupsStrings =>
						push( captures map {case (k, (_, _, c)) => (k, if (c.isInstanceOf[CharSequence]) c.toString else c)} )
					...
					case PushFunctionReferenceInst( entry, name, arity, fidx ) =>
						push( FunctionReference(entry, name, arity, if (frame eq null) Nil else frame.locals.tails.drop(fidx).next) )
						...
						struc match {
							case TupleStructureAST( _, l ) =>
								...
								dereft match {
									...
									case t: Tuple => fail//problem( tpos, s"arity mismatch: expected arity of $arity, but actual arity was ${t.arity}" )
									case o => fail//problem( tpos, s"type mismatch: expected tuple of arity $arity: $o" )
								}
							...
							case RecordStructureAST( _, name, args ) =>
								dereft match {
									...
									case _ => fail//problem( tpos, s"expected a record: '$name'" )//todo: fail with reason???
								}
							case _ =>
						}
					case ListInst( len ) =>
							...
							for (i <- len - 1 to 0 by -1)
								if (op eq null)
									lhs(i) match {
										case l: Assignable =>
											rhs(i) match {
												case `undefined` => problem( rpos(i), "'undefined' may not be assigned to a variable" )
												case r =>
													l.value = r
													res = l.value
											}
													...
													(op, l.value) match {
														...
														case (_, n: Number) =>
															if (!rhs(i).isInstanceOf[Number])
																...
																l.value = Math( func, n, rhs(i) )
														case _ => problem( lpos(i), "illegal assignment" )
													}

													res = l.value
												case _ => problem( lpos(i), s"not an l-value: ${display(lhs(i))}" )
											}
									}
					...
					case GeneratorInst =>
						...
						derefp match {
							case s: String => generate( GeneratorTemp(s.iterator map (_.toString)) )
							case a: Array[_] => generate( GeneratorTemp(a.indices.iterator map (new MutableSeqAssignable(a.asInstanceOf[Array[Any]], _))) )
							case ms: MutableSeq[_] => generate( GeneratorTemp(ms.indices.iterator map (new MutableSeqAssignable(ms.asInstanceOf[MutableSeq[Any]], _))) )
...
//						}
					...
					case BracketInst( epos, apos ) =>
						...
						derefp match {
							...
							case o => problem( epos, s"don't know what to do with '$o' in bracket expression" )
						}
...
case class SectionOperation( op: Symbol, func: FunctionMap )
case class LeftSectionOperation( lpos: Position, l: Any, op: Symbol, func: FunctionMap )
case class RightSectionOperation( op: Symbol, func: FunctionMap, rpos: Position, r: Any )
...
class Record( val name: String, elems: IndexedSeq[Any], val symbolMap: Map[Symbol, Int], val stringMap: Map[String, Int] ) extends TupleLike {
	val arity = elems.length

	def element( idx: Int ) = elems(idx)

	override def toString = if (arity == 0) name else s"$name( ${elems mkString ", "} )"
}
...
case class AssignmentInst( len: Int, lpos: Vector[Position], op: Symbol, func: FunctionMap, rpos: Vector[Position] ) extends VMInst
...
case class BinaryInst( lpos: Position, op: Symbol, func: FunctionMap, rpos: Position ) extends VMInst
case class UnaryInst( op: Symbol, func: FunctionMap, pos: Position ) extends VMInst
